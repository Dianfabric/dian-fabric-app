...
- **2026-09-17 오후 실제 사진 9장 검증** (main @ aa3bbbd): 패턴 원단은 1~3위, 무지·평직·부클은 정확 SKU 실패(질감 유사 원단 수백 개, DINOv2 224 한계). 색상 칩 UI(16색, SearchComparisonView) + 칩 채널 RPC `search_fabrics_v4_color` + 무채색 사진 WB 보정 비교. 칩은 추가 신호로만(대체 시 손해). 카탈로그 색상명 부정확 다수. 다음: 실사진 라벨링→프로젝션 헤드 학습, 재랭킹 후보 확대, 카탈로그 촬영 표준화. 상세 scripts/exp/final-report.md.
