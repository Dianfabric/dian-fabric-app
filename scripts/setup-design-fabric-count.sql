-- list_design_groups RPC: 디자인 수(total_count)에 더해 컬러웨이(원단) 총수(total_fabric_count) 반환.
-- "전체 N개 디자인 · M개 원단" 라벨용. Supabase SQL 에디터에서 1회 실행.
-- 반환 컬럼이 바뀌므로 기존 함수 DROP 먼저 (create or replace는 반환타입 변경 불가).

drop function if exists list_design_groups(text, text[], text, boolean, text, text, int, int, numeric, numeric, numeric);
drop function if exists list_design_groups(text, text[], text, boolean, text, text, int, int);
