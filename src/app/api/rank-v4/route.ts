...
쿼리 사진은 조명·각도·확대 배율이 카탈로그와 다를 수 있으니 원근 왜곡과 약한 조명 편차는 무시하되, 다음 순서로 엄격하게 판단하세요:
1. 색상 계열 (가장 중요. 쿼리가 퍼플이면 퍼플·보라 계열만 높은 점수. 그레이·베이지 등 다른 계열은 질감이 같아도 40점 이하)
2. 패턴 종류 (스트라이프·체크·무지·기하학 등. 쿼리가 스트라이프인데 후보가 무지이면 40점 이하)
3. 직조/질감 (조직, 표면 결, 광택, 패턴의 상대적 크기)

후보 ${n}개 전부를 쿼리와 비슷한 순서로 정렬하고 0~100 점수를 주세요 (100=같은 원단, 70~89=같은 색상 계열·같은 패턴의 비슷한 원단, 40 이하=색상 계열 또는 패턴 종류가 다름).
