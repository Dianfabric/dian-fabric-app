    ...
    <>
      ...
      <section className="min-h-screen flex items-center justify-center text-center px-10 pt-20 pb-20 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-[17px] text-gray-400 leading-relaxed max-w-[560px] mx-auto mb-10">
            사진 한 장이면 충분합니다. AI가 색상·텍스쳐·패턴을 종합 분석해
            14,625개 원단에서 가장 비슷한 것을 찾아드립니다.
          </p>
      ...
      <div className="max-w-[800px] mx-auto -mt-10 relative z-20 bg-white rounded-[20px] px-12 py-8 grid grid-cols-3 gap-8 shadow-[0_8px_40px_rgba(0,0,0,0.06)]">
        {[
          { num: "14,625+", label: "프리미엄 원단" },
          { num: "1,629", label: "수동 컨펌 분류" },
          { num: "4단계", label: "AI 매칭 파이프라인" },
        ].map((stat) => (
          ...
          {[
            {
              ...
              icon: "📸",
              title: "이미지 업로드",
              desc: "원하는 느낌의 사진을 올려주세요. 인테리어 컷, 핀터레스트, 원단 샘플 모두 OK.",
            },
            {
              num: "2",
              icon: "📊",
              title: "다중 AI 분석",
              desc: "Gemini가 패턴·색상을 분류하고, DINOv2(768차원)가 텍스쳐를, LAB 색공간이 사람 눈 기준 색을 추출합니다.",
            },
            {
              num: "3",
              icon: "🎯",
              title: "Soft Scoring 매칭",
              desc: "14,625개 원단 중 색상·텍스쳐·1,629개 수동 분류 데이터를 가중치 합산해 후보를 추립니다.",
            },
            {
              num: "4",
              icon: "✨",
              title: "결과 비교 & 컬러 옵션",
              desc: "Top 15 매칭 + 같은 시리즈 컬러 변형까지 한눈에. 화살표 키로 빠르게 비교하세요.",
            },
          ].map((step) => (
            <div
              key={step.num}
              className="bg-white rounded-[20px] p-7 text-center border border-gray-100 card-hover"
            >
      ...
      <section className="max-w-[1100px] mx-auto px-10 py-12">
        <div className="bg-[linear-gradient(135deg,rgba(139,105,20,0.04),rgba(196,154,108,0.04))] rounded-3xl p-12">
          <p className="text-[13px] font-bold text-gradient text-center mb-2">TECH STACK</p>
          <h2 className="text-[28px] font-extrabold text-center mb-10 tracking-tight">
            업계 최고 수준 AI 조합
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            {[
              { name: "DINOv2", desc: "Meta 자기지도 모델\n텍스쳐 768차원" },
              { name: "LAB ΔE", desc: "인쇄·도장 산업 표준\n사람 눈 색 매칭" },
              { name: "Gemini 3 Flash", desc: "이미지 멀티모달 분석\n패턴·색상 분류" },
              { name: "Multi-Scale", desc: "전체 + 중앙 크롭\n미세 짜임 캡처" },
            ].map((tech) => (
              <div key={tech.name} className="bg-white rounded-2xl p-6">
                <div className="font-extrabold text-[#8B6914] text-base mb-2">{tech.name}</div>
                <div className="text-[12px] text-gray-500 whitespace-pre-line leading-relaxed">{tech.desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="text-center px-10 py-24">
        <div className="max-w-[640px] mx-auto bg-[linear-gradient(135deg,#1a1a1a,#2D2520)] rounded-3xl px-12 py-16">
          ...
          <p className="text-[15px] text-gray-500 mb-8">
            14,625개 프리미엄 원단에서 당신만의 원단을 찾아보세요
          </p>
          <Link
            href="/search"
